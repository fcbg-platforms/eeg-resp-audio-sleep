...
# %% Filter entire buffer with steady-state response estimate
...
# %% Filter in chunks with steady-state response estimate
fname = root / "synchronous-respiration-raw.fif"
raw = read_raw_fif(fname, preload=True)
events = find_events(raw)
events = events[events[:, 2] == 3]  # only keep the target events
...
for start in range(0, data.shape[0], 10):
    ...
    for filt in filters:
        if filt["zi"] is None:
            filt["zi"] = np.mean(chunk, axis=0) * filt["zi_unit"]
        chunk, filt["zi"] = sosfilt(filt["sos"], chunk, axis=0, zi=filt["zi"])
    data_filtered[start:stop] = chunk

data = data_filtered.squeeze()
z = np.polyfit(raw.times, data, 1)
data -= z[0] * raw.times + z[1]
peaks = find_peaks(data, distance=0.8 * raw.info["sfreq"], height=np.mean(data))[0]
